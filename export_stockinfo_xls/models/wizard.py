...
class StockReport(models.TransientModel):
    ...
    def get_lines(self, data, warehouse):
        ...
        for obj in stock_history:
            ...
            available_qty = product.virtual_available - product.incoming_qty
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range(7, 7, 7, count - 1, 'Warehouses', format1)
        sheet.merge_range('A9:G9', 'Product Information', format11)
        w_col_no = 6
        w_col_no1 = 7
        for i in get_warehouse[0]:
            w_col_no = w_col_no + 11
            sheet.merge_range(8, w_col_no1, 8, w_col_no - 1, i, format11)
            w_col_no1 = w_col_no1 + 11
        sheet.write(9, 0, 'Item', format21)
        sheet.merge_range(9, 1, 9, 3, 'Description', format21)
        ...
        for i in get_warehouse[0]:
            ...
            sheet.write(9, p_col_no1 + 1, 'Incoming', format21)
            sheet.write(9, p_col_no1 + 2, 'Outgoing', format21)
            sheet.merge_range(9, p_col_no1 + 3, 9, p_col_no1 + 4, 'Net On Hand', format21)
            sheet.merge_range(9, p_col_no1 + 5, 9, p_col_no1 + 6, 'Total Sold', format21)
            sheet.merge_range(9, p_col_no1 + 7, 9, p_col_no1 + 8, 'Total Purchased', format21)
            sheet.write(9, p_col_no1 + 9, 'Valuation', format21)
        ...
        for i in get_warehouse[1]:
            ...
            for each in get_line:
                ...
                if each['incoming'] < 0:
                    sheet.write(prod_row, prod_col + 1, each['incoming'], red_mark)
                else:
                    sheet.write(prod_row, prod_col + 1, each['incoming'], font_size_8)
                if each['outgoing'] < 0:
                    sheet.write(prod_row, prod_col + 2, each['outgoing'], red_mark)
                else:
                    sheet.write(prod_row, prod_col + 2, each['outgoing'], font_size_8)
                if each['net_on_hand'] < 0:
                    sheet.merge_range(prod_row, prod_col + 3, prod_row, prod_col + 4, each['net_on_hand'], red_mark)
                else:
                    sheet.merge_range(prod_row, prod_col + 3, prod_row, prod_col + 4, each['net_on_hand'], font_size_8)
                    if each['sale_value'] < 0:
                        sheet.merge_range(prod_row, prod_col + 5, prod_row, prod_col + 6, each['sale_value'], red_mark)
                    else:
                        sheet.merge_range(prod_row, prod_col + 5, prod_row, prod_col + 6, each['sale_value'],
                                          font_size_8)
                if each['purchase_value'] < 0:
                    sheet.merge_range(prod_row, prod_col + 7, prod_row, prod_col + 8, each['purchase_value'], red_mark)
                else:
                    sheet.merge_range(prod_row, prod_col + 7, prod_row, prod_col + 8, each['purchase_value'],
                                      font_size_8)
                if each['total_value'] < 0:
                    sheet.write(prod_row, prod_col + 9, each['total_value'], red_mark)
                else:
                    sheet.write(prod_row, prod_col + 9, each['total_value'], font_size_8_r)
                prod_row = prod_row + 1
            prod_row = 10
